...
final class AppController: ObservableObject {
    ...
    func testAirplane() {
        ...
        showAirplane(for: fake, minutesUntil: 10)
    }
    ...
    private func showAirplane(for event: CalendarEvent, minutesUntil: Int) {
        ...
        DispatchQueue.main.async {
            ...
            // If user clicks the airplane, dismiss this meeting permanently
            window.onDismissed = { [weak self, weak window] in
                self?.poller?.dismissedIDs.insert(event.id)
                if let w = window {
                    self?.overlayWindows.removeAll { $0 === w }
                    w.close()
                }
            }
